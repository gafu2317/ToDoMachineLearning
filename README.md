...
- ✅ **リアルタイム学習**（オンライン強化学習、学習/テストモード分離）
- ✅ **隠れパラメータシステム**（部分観測環境での強化学習）
...
- 状態空間: タスク数、平均重要度、平均時間、最短締切、集中力、**疲労蓄積度**（6次元）
- 行動空間: どのタスクを実行するか
- 報酬関数:
  - 基本完了報酬: スコア × 0.1
  - 締切遵守ボーナス: 早期完了で最大+20
...
## 新機能：リアルタイム学習と隠れパラメータシステム

### リアルタイム学習（オンライン強化学習）

強化学習スケジューラーが**実行中に継続的に学習**するオンライン学習システムを実装。

#### 学習モード vs テストモード

- **学習モード**: シミュレーション実行中にQ値が更新される（epsilon=0.5）
- **テストモード**: Q値は更新されない（epsilon=0.0、完全greedy）
...
エージェントが観測できない情報を環境に持たせることで、**部分観測環境**での強化学習を実現。

#### 隠れパラメータの種類

1. **真の疲労係数（decay_factor）**
   - エージェントは現在の集中力レベルのみを観測
   - 真の減衰速度は隠されている
   - 5つのバリエーション（very_short～very_long）

2. **タスク時間の変動率**
   - 予定時間より±20～40%変動
   - 正規分布に従う（mean=1.0, std=0.15）
   - エージェントは基本所要時間のみを知る
...
強化学習の状態空間に**疲労蓄積度**を追加：

- **従来**: (タスク数, 重要度比率, 締切, 平均時間, 集中力)
- **新規**: (タスク数, 重要度比率, 締切, 平均時間, 集中力, **疲労蓄積度**)

疲労蓄積度は`連続作業時間 / 最大作業時間`で計算され、0.0～1.0の値を取る。
