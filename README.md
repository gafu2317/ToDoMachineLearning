...
- 30%のタスクが依存関係を持つ
- 各タスクは1-2個の依存先タスクを持つ
- 後のタスクが前のタスクに依存（現実的な制約）
- 依存先タスクが全て完了するまで実行不可
- 締切は依存先より必ず後になるよう自動調整
```

**実装のポイント：**
- `Task.is_ready_to_start(completed_task_ids)` で依存関係を確認
- 全スケジューラで依存関係を満たすタスクのみを選択候補とする
- Q-learningの状態空間は変更せず、既存モデルとの互換性を維持
- 依存関係フィルタリング後のタスクから選択するため、状態表現に明示的に含める必要なし

**依存関係導入による効果：**
- より現実的なタスクスケジューリング環境を実現
