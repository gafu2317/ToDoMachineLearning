...
タスクの実際の所要時間は `base_duration × _hidden_duration_multiplier` で決まる。スケジューラーにはこの変動率は開示しない。

| パラメータ | 分布・範囲 | 状態 |
|---|---|---|
| 時間変動率 | N(1.0, 0.15²)、[0.8, 1.4] でクリップ | 有効（唯一の実効パラメータ） |
| ジャンル適性 | [-0.2, 0.3] の一様分布 | 未使用（タスクに付与されるが読み取られていない） |

RL スケジューラーは実際の実行を通じてこの変動を経験し学習していく。
...
- **スケジュール比較グラフ**: 4手法の予定スケジュールを並べたガンツチャート
- **Planned vs Actual**: 各スケジューラーの予定と実際（隠しパラメータ付き）の対比グラフ
  - 3週間分生成し、RL の Q-table が週間を通じて継続学習していく過程を可視化
